...
from tsl.style import get_color, get_current_style
...
class TSLStyle(QProxyStyle):
    ...
    class QCheckBoxOptions:  # pylint: disable=too-few-public-methods
        ...
        def text_option(option: QStyleOptionButton) -> QStyleOptionButton:
            ...
            offset = option.iconSize.width() + TSLStyle.QCheckBoxOptions.SPACER
    ...
    class ToggleOptions:  # pylint: disable=too-few-public-methods
        ...
        def toggle_x(option: QStyleOption) -> int:
            ...
            if option.direction == Qt.LeftToRight:
                return 0
            # cast Qt.RightToLeft
            return option.rect.width() - TSLStyle.ToggleOptions.BOX_WIDTH
        ...
        def text_option(option: ToggleOptionButton) -> QStyleOptionButton:
            ...
            text_option = QStyleOptionButton(option)
            text_option.rect = QRect(
                TSLStyle.ToggleOptions.label_x(option),
                0,
                option.rect.width()
                - TSLStyle.ToggleOptions.BOX_WIDTH
                - TSLStyle.ToggleOptions.SPACER,
        ...
        def toggle_rect(option: ToggleOptionButton) -> QRect:
            ...
            return QRect(
                TSLStyle.ToggleOptions.toggle_x(option),
                0,
                TSLStyle.ToggleOptions.BOX_WIDTH,
    ...
    def standardPalette(  # pylint: disable=invalid-name,no-self-use
        ...
        try:
            return TSLStyle.PALETTE_CACHE[style]
        except KeyError:
            TSLStyle.PALETTE_CACHE[style] = self._create_palette()
            return TSLStyle.PALETTE_CACHE[style]

    def _create_palette(self) -> QPalette:
        """Create a Palette for the color names for the current style."""
        palette = super().standardPalette()
        for group, role, name in TSLStyle.PALETTE_COLOR_NAMES:
    ...
    def _draw_checkbox(
        ...
        text_option = TSLStyle.QCheckBoxOptions.text_option(option)
    ...
    def _draw_toggle(
        ...
        if option.text:
            self.drawControl(
                ...
                TSLStyle.ToggleOptions.text_option(option),
                painter,
                widget,
            )
    ...
    def _draw_toggle_circle(
        ...
        with painter_save(painter):
            ...
            painter.setBrush(QColor(TSLStyle.button_foreground(option)))
            painter.drawEllipse(
                TSLStyle.ToggleOptions.toggle_x(option) + option.position,
                TSLStyle.ToggleOptions.CIRCLE_OFFSET,
    ...
    def _draw_toggle_background(
        ...
        with painter_save(painter):
            painter.setPen(Qt.NoPen)
            painter.setBrush(QColor(TSLStyle.button_background(option)))
            painter.drawRoundedRect(
                TSLStyle.ToggleOptions.toggle_rect(option),
    ...
    def _draw_primitive_indicator_checkbox(
        ...
        """Draw a checkbox."""
        TSLStyle._draw_checkbox_background(option, painter)
        TSLStyle._draw_checkbox_frame(option, painter)
        ...
        if (
            ...
            TSLStyle._draw_checkbox_check(option, painter)
    ...
    def _draw_checkbox_background(
        ...
        with painter_save(painter):
            ...
            painter.setPen(Qt.NoPen)
            painter.setBrush(QBrush(TSLStyle._cb_background_color(option)))
    ...
    def _draw_checkbox_frame(
        ...
        with painter_save(painter):
            ...
            painter.setPen(QPen(TSLStyle._cb_frame_color(option)))
    ...
    def _draw_checkbox_check(
        ...
        TSLStyle.draw_pixmap(
            painter,
            option.rect.adjusted(1, 1, -1, -1),
            TSLStyle.QCheckBoxOptions.ICON_PATH[state],
            TSLStyle.button_foreground(option).name(),
        )
