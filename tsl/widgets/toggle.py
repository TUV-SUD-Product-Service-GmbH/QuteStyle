...
class Toggle(QCheckBox, TextTruncator):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.setFont(
            QFont(
                TSLStyle.ToggleOptions.FONT_NAME,
                TSLStyle.ToggleOptions.FONT_SIZE,
            )
        )

        # The minimum size is defined by the toggle box's dimensions.
        self.setMinimumHeight(TSLStyle.ToggleOptions.BOX_HEIGHT)
        self.setMinimumWidth(TSLStyle.ToggleOptions.BOX_WIDTH)

        self.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed)

        self.setCursor(Qt.PointingHandCursor)

        self._position = TSLStyle.ToggleOptions.CIRCLE_OFFSET

        self._animation = QPropertyAnimation(self, b"position")
        self._animation.setEasingCurve(QEasingCurve.OutBounce)
        self._animation.setDuration(TSLStyle.ToggleOptions.ANIM_DURATION)
    ...
    def sizeHint(self) -> QSize:  # pylint: disable=invalid-name
        ...
        return QSize(width, TSLStyle.ToggleOptions.BOX_HEIGHT)
    ...
    def setup_animation(self, value: Qt.CheckState) -> None:
        ...
        if value != Qt.Unchecked:
            ...
            end = TSLStyle.ToggleOptions.CIRCLE_OFFSET
    ...
    def paintEvent(  # pylint: disable=invalid-name
        ...
        painter.drawControl(TSLStyle.CE_Toggle, option)
        painter.end()
