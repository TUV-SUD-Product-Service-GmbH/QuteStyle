...
from tsl.helper import create_waiting_spinner
from tsl.init import check_ide
from tsl.startup_threads import StartupThread
from tsl.style import get_color, get_style
...
class TslApplication(  # pylint: disable=too-many-instance-attributes
    QApplication
):
    """TslApplication."""
    ...
    def __init__(self, argv: List[str], show_splash: bool = True):
        ...
        self.setOrganizationName("TÜV SÜD Product Service GmbH")
        self.setOrganizationDomain("tuvsud.com")
        if show_splash:
        ...
        if not self._update and Updater in self._threads_to_run:
            # do not run updater thread in this case
            log.debug("Removing update thread from start threads")
            self._threads_to_run.remove(Updater)
    ...
    def _run_threads(self) -> None:
        ...
        for thread_class in copy(self._threads_to_run):
            ...
            if not all(
                dependency in threads_finished
    ...
    def on_finished_thread(self) -> None:
        ...
        if not self._threads_running and not self._threads_to_run:
            ...
            if exit_threads:
                ...
                log.debug(
                    "There are no threads left to run, starting MainWindow."
                )
                self.show_main_window()
        else:
