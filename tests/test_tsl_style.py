...
from tsl.dev.mocks import CallList, check_call, check_call_str
...
@pytest.mark.parametrize(
    ...
    (
        TSLStyle.CE_Toggle,
        TSLStyle.CE_CheckBoxLabel,
        TSLStyle.CE_CheckBox,
        TSLStyle.CE_ItemViewItem,
    ),
    ids=(
        "TSLStyle.CE_Toggle",
        "TSLStyle.CE_CheckBoxLabel",
...
def test_draw_control(element: TSLStyle.ControlElement) -> None:
    """Test that drawControl calls the correct sub functions."""
    method_name = {
        TSLStyle.CE_Toggle: "_draw_toggle",
        TSLStyle.CE_CheckBoxLabel: "_draw_check_box_label",
        TSLStyle.CE_CheckBox: "_draw_checkbox",
        TSLStyle.CE_ItemViewItem: None,
    }[element]
    if not method_name:
        with check_call(QProxyStyle, "drawControl"):
            TSLStyle().drawControl(element, QStyleOption(), QPainter(), None)
    else:
        with check_call(TSLStyle, method_name):
            option = (
                ToggleOptionButton
                if element == TSLStyle.CE_Toggle
                else QStyleOptionButton
            )
            widget = QCheckBox() if element == TSLStyle.CE_CheckBox else None
            TSLStyle().drawControl(element, option(), QPainter(), widget)
...
def test_draw_check_box(style_option_button: QStyleOptionButton) -> None:
    ...
    with check_call(TSLStyle, "_draw_indicator_checkbox"):
        with check_call(TSLStyle, "drawControl") as draw_control_calls:
            TSLStyle()._draw_checkbox(
...
def test_draw_checkbox_label(
    ...
    with check_call(QProxyStyle, "drawControl") as calls:
        TSLStyle()._draw_check_box_label(style_option_button, QPainter(), None)
    option = calls[0][0][2]
...
class TestDrawIndicatorCheckbox:
    ...
    def calls(
        ...
        with check_call(QProxyStyle, "subElementRect", rect):
            with check_call(TSLStyle, "drawPrimitive") as calls:
                TSLStyle()._draw_indicator_checkbox(
...
def test_draw_toggle(
    ...
    with check_call(TSLStyle, "_draw_toggle_background"):
        with check_call(TSLStyle, "_draw_toggle_circle"):
            with check_call(
                QProxyStyle, "drawControl", call_count=1 if text else 0
            ):
                TSLStyle()._draw_toggle(toggle_option_button, QPainter(), None)
...
class TestDrawToggleCircle:
    ...
    def calls(
        ...
        """Call the method and return the calls to drawPrimitive."""
        with check_call(TSLStyle.ToggleOptions, "toggle_x", toggle_x):
            with check_call(QPainter, "drawEllipse") as calls:
                TSLStyle()._draw_toggle_circle(toggle_option_button, painter)
        return calls
    ...
    def test_y_pos(calls: CallList) -> None:
        ...
        assert calls[0][0][2] == TSLStyle.ToggleOptions.CIRCLE_OFFSET
    ...
    def test_radius(calls: CallList) -> None:
        ...
        assert (
            calls[0][0][3]
            == calls[0][0][4]
            == TSLStyle.ToggleOptions.CIRCLE_SIZE
        )
...
class TestDrawToggleBackground:
    ...
    def calls(
        ...
        """Call the method and return the calls to drawPrimitive."""
        with check_call(TSLStyle.ToggleOptions, "toggle_rect", rect):
            with check_call(QPainter, "drawRoundedRect") as calls:
                TSLStyle()._draw_toggle_background(
    ...
    def test_radius(calls: CallList) -> None:
        ...
        assert (
            calls[0][0][2]
            == calls[0][0][3]
            == TSLStyle.ToggleOptions.BACKGROUND_RECT_RADIUS
        )


class TestDrawPrimitive:
    """Test drawing a primitive element with TSLStyle."""
    ...
    def test_frame_focus_rect() -> None:
        ...
        with check_call(QProxyStyle, "drawPrimitive", call_count=0):
            TSLStyle().drawPrimitive(
                TSLStyle.PE_FrameFocusRect, QStyleOption(), QPainter(), None
            )
    ...
    def test_panel_item_view_item(
        ...
        with check_call(TSLStyle, "_panel_draw_item_view_item"):
            TSLStyle().drawPrimitive(
                TSLStyle.PE_PanelItemViewItem,
    ...
    def test_draw_primitive_indicator_checkbox(
        ...
        with check_call(TSLStyle, "_draw_primitive_indicator_checkbox"):
            TSLStyle().drawPrimitive(
                TSLStyle.PE_IndicatorCheckBox,
                style_option_view_item,
                QPainter(),
                None,
            )
...
def test_draw_primitive_indicator_checkbox(
    ...
    with check_call(TSLStyle, "_draw_checkbox_background"):
        with check_call(TSLStyle, "_draw_checkbox_frame"):
            draw_check = (
            ...
            with check_call(
                ...
                TSLStyle._draw_primitive_indicator_checkbox(
...
class TestDrawCheckBoxFrame:
    ...
    def calls(
        ...
        """Test drawing a checkbox's frame."""
        with check_call_str("tsl.tsl_style.painter_save", painter_save_mock):
            with check_call(QPainter, "drawRoundedRect") as calls:
                TSLStyle()._draw_checkbox_frame(style_option_button, painter)
        return calls
    ...
    def test_pen(
        ...
        assert painter.pen() == QPen(
            TSLStyle._cb_frame_color(style_option_button)
        )
...
class TestDrawCheckBackground:
    ...
    def calls(
        ...
        with check_call_str("tsl.tsl_style.painter_save", painter_save_mock):
            with check_call(QPainter, "drawRoundedRect") as calls:
                TSLStyle()._draw_checkbox_background(
    ...
    def test_brush(
        ...
        assert painter.brush() == QBrush(
            TSLStyle._cb_background_color(style_option_button)
        )
